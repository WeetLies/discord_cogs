...
class Craw_royalSurgery(commands.Cog):
    def __init__(self,weetlies):
        self.weetlies = weetlies
    
    @commands.command(aliases=['성형','로얄성형'])
    async def royalsurgery(self,ctx):
        html = urlopen("https://maplestory.nexon.com/Guide/CashShop/Probability/RoyalPlasticSurgeryCoupon")
        ...
        embed=discord.Embed(title="현재 판매중인 로얄성형쿠폰 확률이에요",color=0x000000)
        embed.add_field(name="로얄성형쿠폰",value="🧑🏻",inline=False)
...
def setup(weetlies):
    weetlies.add_cog(Craw_royalSurgery(weetlies))